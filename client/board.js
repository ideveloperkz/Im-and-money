...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (finModal) {
        finModal.addEventListener("click", (e) => {
            if (e.target === e.currentTarget) finModal.classList.remove("active");
        });
    }
});
...
function loadFinanceDataOnOpen() {
    ...
    // НЕ МЕНЯТЬ на false, иначе карточка будет открываться пустой!
...
function refreshFinanceData(updateManualInputs = false) {
    ...
    socket.emit('player:get_finance_data', (res) => {
        ...
        if (window.gameClient && window.gameClient.myPlayerData) {
            updateFinanceCardHeader(window.gameClient.myPlayerData);
        }

        // 1. Обновляем общие элементы (всегда)
        updateHistoryTable(res.turnHistory || []);
        updateAssetsList(res.assets || {}); // Pass full assets object (businesses + items)
