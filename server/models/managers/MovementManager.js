...
class MovementManager {
    ...
    movePlayer(playerId, steps) {
        ...
        if (board[currentCell].type === 'fork' && player.forkDirection !== null) {
            ...
            // Проверяем, не попали ли мы сразу на "Деньги"
        ...
        for (let i = 0; i < steps; i++) {
            ...
            // Если мы ПРОХОДИМ (не останавливаясь) через клетку "Деньги"
        ...
        // Записываем ход в историю игрока (Turn History)
        const turnEntry = {
            turnNumber: player.turnHistory.length + 1,
            dice: steps, // Исходное кол-во шагов (примерно)
            cellKey: currentCell,
