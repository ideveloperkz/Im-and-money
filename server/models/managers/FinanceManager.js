...
class FinanceManager {
    ...
    collectBusinessIncome(playerId) {
        ...
        if (player.assets.businesses && player.assets.businesses.length > 0) {
            player.assets.businesses.forEach(business => {
                const cashflow = Number(business.income) || Number(business.cashflow) || Number(business.monthlyIncome) || 0;
    ...
    buyBusiness(playerId, businessData) {
        ...
        // Парсим доход
        let incomeAmount = businessData.income !== undefined ? businessData.income : (businessData.cashflow !== undefined ? businessData.cashflow : (businessData.monthlyIncome || 0));
