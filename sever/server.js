...
io.on('connection', (socket) => {
    ...
    socket.on('player:draw_card_from_deck', (data, callback) => {
        ...
        try {
            ...
            // Проверяем ДО отправки game:card_drawn, чтобы не показывать модалку карты
            if (card.skillCheckFailed || card.assetCheckFailed) {
                ...
                if (card.assetCheckFailed) {
                    ...
                    notificationMessage = `❌ У игрока ${gameState.players[playerId].displayName} нет предмета "${card.offer_asset_name || 'актив'}" для продажи.`;
                } else if (card.skillCheckFailed && card.requiresSkill) {
                    // Нет навыка
                    const skillName = gameState.getSkillDisplayName(card.requiresSkill);
                    notificationMessage = `❌ У игрока ${gameState.players[playerId].displayName} нет навыка "${skillName}". Доход не зачислен.`;
                }

                if (notificationMessage) {
                    // Отправляем уведомление ВСЕМ (БЕЗ модалки карты)
                    io.emit('game:notification', {
                    ...
                    gameState.nextTurn();
                    io.emit('game:state_update', gameState.getState());
                    return; // ВАЖНО: Выходим, не показываем карту
                }
            }
    ...
    /**
     * ИГРОК: Подтвердил просмотр карточки (ОК)
     * Используется для карточек без выбора, но требующих внимания
     */
    ...
    socket.on('player:purchase_choice', (data, callback) => {
        ...
        try {
            ...
            const messages = lastCard.messages || {};
            ...
            if (accept) {
                ...
                if (currentBalance >= price) {
                    ...
                    if (isAsset) {
                        ...
                        // Сообщение
                        const template = messages.msg_success_log || messages.msg_success_others || `📦 {Player} приобрел "{CardName}" за {Amount}₴!`;
                        ...
                        // Сообщение
                        const template = messages.msg_success_log || messages.msg_success_others || `🏢 {Player} купил бизнес: {CardName}!`;
                        const message = interpolateMessage(template, params);
                ...
                const template = messages.msg_decline_log || messages.msg_decline_others || `Игрок {Player} отказался от покупки {CardName}.`;
                const message = interpolateMessage(template, params);
    ...
    socket.on('player:charity_choice', (data, callback) => {
        ...
        try {
            ...
            const params = {
                Player: player.displayName,
                CardName: lastCard.title || 'Благотворительность',
                Amount: donationAmount
            };

            if (accept) {
                if (wallets.charity >= donationAmount) {

                    // === SPECIAL LOGIC: Chance #7 (Buff instead of payment) ===
                    if (lastCard.id === 'chance_help_transport' && lastCard.buff) {
                        const buffData = {
                        ...
                        const template = messages.msg_success_log || messages.msg_success || 'Игрок {Player} помог другу.';
                        const message = interpolateMessage(template, params);
                        ...
                        // Обычная благотворительность (списание денег)
                        ...
                        player.charityDonationsMade = (player.charityDonationsMade || 0) + 1;

                        const template = messages.msg_success_log || messages.msg_success_others || `💝 {Player} пожертвовал {Amount}₴ на благотворительность!`;
                        const message = interpolateMessage(template, params);
                ...
                const template = messages.msg_decline_log || messages.msg_decline_others || `{Player} отказался от пожертвования`;
                const message = interpolateMessage(template, params);
